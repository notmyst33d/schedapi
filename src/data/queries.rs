...
impl Queries {
    pub async fn new(session: &Session) -> Result<Self, Box<dyn Error + 'static>> {
        Ok(Self {
            ...
            add_group: session.prepare("INSERT INTO groups (id, epoch, name, schedule2) VALUES (?, ?, ?, ?)").await?,
            ...
            update_schedule: session.prepare("UPDATE groups SET schedule2 = ? WHERE id = ?").await?,
